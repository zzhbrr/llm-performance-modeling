...
# Workload Spec
batchsize = 1
seql = 1024 * 5 # sequence length
# seql = 102 # sequence length
cache_hit_ratio = 0.2
cache_hit_length = seql * cache_hit_ratio
compute_seql = seql - cache_hit_length
...
def PrintConfig():
    ...
    print("    cache_hit_length: ", int(cache_hit_length))
    print("    compute_seql: ", int(compute_seql))
    print("Model: ")
...
def analyze():
    for op in Operations:
        if op == "Q": 
            record("prefill", 
                   op, 
                   OPs = 2 * compute_seql * batchsize * d * d / tp_size, 
                   load_weight = d * d * w_byte / tp_size, 
                   load_act = compute_seql * batchsize * d * a_byte, 
                   store_act = compute_seql * batchsize * d * a_byte / tp_size, 
                   load_kv_cache = 0, 
                   store_kv_cache = 0)
        elif op == "K" or op == "V": 
            record("prefill", 
                   op, 
                   OPs = 2 * compute_seql * batchsize * d * h_kv * headsize / tp_size, 
                   load_weight = d * h_kv * headsize * w_byte / tp_size, 
                   load_act = compute_seql * batchsize * d * a_byte, 
                   store_act = 0, 
                   load_kv_cache = 0, 
                   store_kv_cache = compute_seql * batchsize * h_kv * headsize * kv_byte / tp_size)
        elif op == "qk_matmul":
            record("prefill", 
                   op, 
                   OPs = 2 * batchsize * compute_seql * seql * h * headsize / tp_size, 
                   load_weight = 0, 
                   load_act = compute_seql * batchsize * h * headsize * a_byte / tp_size, #  Q
                   store_act = batchsize * compute_seql * seql * h * a_byte / tp_size, 
                   load_kv_cache = seql * headsize * batchsize * h_kv * kv_byte / tp_size, 
                   store_kv_cache=0)
        elif op == "softmax":
            record("prefill", 
                   op, 
                   OPs = 5 * batchsize * compute_seql * seql * h / tp_size, 
                   load_weight=0,
                   load_act=batchsize * compute_seql * seql * h * a_byte / tp_size, 
                   store_act=batchsize * compute_seql * seql * h * a_byte / tp_size, 
                   load_kv_cache=0, 
                   store_kv_cache=0)
        elif op == "sv_matmul":
            record("prefill", 
                   op, 
                   OPs = 2 * batchsize * compute_seql * seql * headsize * h / tp_size,
                   load_weight=0, 
                   load_act=batchsize * compute_seql * seql * h * a_byte / tp_size, 
                   store_act=batchsize * compute_seql * h * headsize * a_byte / tp_size, 
                   load_kv_cache=batchsize * seql * headsize * h_kv * kv_byte / tp_size, 
                   store_kv_cache=0)
        elif op == "O":
            record("prefill", 
                   op, 
                   OPs = 2 * batchsize * compute_seql * d * d / tp_size, 
                   load_weight=d * d * w_byte / tp_size, 
                   load_act=batchsize * compute_seql * d * a_byte / tp_size, 
                   store_act=batchsize * compute_seql * d * a_byte / tp_size, 
                   load_kv_cache=0, 
                   store_kv_cache=0)
        elif op == "FFN_up" or op == "FFN_gate":
            record("prefill", 
                   op, 
                   OPs = 2 * batchsize * compute_seql * d * d_ffn / tp_size, 
                   load_weight=d * d_ffn * w_byte / tp_size, 
                   load_act=batchsize * compute_seql * d * a_byte, 
                   store_act=batchsize * compute_seql * d_ffn * a_byte / tp_size, 
                   load_kv_cache=0, 
                   store_kv_cache=0)
        elif op == "FFN_down":
            record("prefill", 
                   op, 
                   OPs = 2 * batchsize * compute_seql * d_ffn * d / tp_size, 
                   load_weight=d_ffn * d * w_byte / tp_size, 
                   load_act=batchsize * compute_seql * d_ffn * a_byte / tp_size, 
                   store_act=batchsize * compute_seql * d * a_byte, 
                   load_kv_cache=0, 
                   store_kv_cache=0)

def GetKVCacheSizePerLayer(): # bytes
    return 2 * seql * batchsize * h_kv * headsize * kv_byte
def GetHitKVCacheSizePerLayer(): # bytes
    return 2 * seql * batchsize * h_kv * headsize * kv_byte * cache_hit_ratio
...
if __name__ == "__main__":
    ...
    for stage in ['prefill', 'decode']:
        if stage == 'decode': continue
        print("Stage:", stage)
        t[stage] = 0
        if tp_size != 1:
            if stage == 'prefill':
                volume = 2 * compute_seql * batchsize * d * a_byte # 一次allreduce的bytes
    ...
    print("Cache Hit KV Cache Size per layer: ", round(GetHitKVCacheSizePerLayer() / 1024 / 1024 / 1024, 3), "GB")
